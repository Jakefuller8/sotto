...
async function main() {
  ...
  try {
    ...
    const first = await post("/pair?room=PAIR23", { role: "laptop", pub: laptop.pub });
    ...
    const second = await post("/pair?room=PAIR23", { role: "phone", pub: phone.pub });
    ok("hands the phone the laptop's key", second.body.peer === laptop.pub);

    const both = await get("/pair?room=PAIR23");
    ...
    let pending = get("/poll?room=PAIR23");
    await sleep(150);
    const sent = await post("/say?room=PAIR23", { iv: b64(iv), ct: b64(ct) });
    ...
    await post("/say?room=UNI234", { iv: b64(iv2), ct: b64(ct2) });
    ...
    await post("/say?room=LON234", { iv: b64(iv3), ct: b64(ct3) });
    ...
    const orphan = await post("/say?room=NOB234", { iv: "i", ct: "void" });
    ...
    await sleep(500);
    await post("/say?room=TIM234", { iv: "i", ct: "quick" });
    await pending;
    ...
    ok("stats contain no room codes",
      !JSON.stringify(stats.body).includes("PAIR23"));
    ok("logs are event-only (hashed user ids)",
      serverOut.includes("event=dictation") && !serverOut.includes("PAIR23"),
      serverOut.split("\n").slice(0, 3).join(" | "));
